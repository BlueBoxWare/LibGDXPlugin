...
class GdxJsonElementFactory(private val project: Project) {
  ...
  private inline fun <reified T: GdxJsonElement> createElement(content: String): T? =
          createFile(content)?.childOfType()

  private fun createFile(content: String) =
          PsiFileFactory.getInstance(project).createFileFromText("dummy.json", LibGDXJsonFileType.INSTANCE, content) as? GdxJsonFile
